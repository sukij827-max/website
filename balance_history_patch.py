...
def balance_history_html(uid):
    ...
    for t in tx:
        ...
        rows.append(
            ...
            f'<div><b>{label}</b><div class="muted small">{t.created_at} · {t.note or ""}</div></div>'
    ...
    return (
        ...
        f'<div class="big">Rp {Decimal(u.balance or 0):,.0f}</div></div>'
...
BALANCE_HISTORY_CSS = '''
<style>
.balance-live-head{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:14px 0;border-bottom:1px solid #293750;margin-bottom:8px}
.live-dot{font-size:11px;font-weight:800;color:#6ee7a3}
.balance-history-list{display:grid;gap:7px;max-height:520px;overflow:auto}
.balance-tx{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px;border-radius:12px;background:#111a2a;border:1px solid #293750}
.balance-tx.plus .balance-amount{color:#6ee7a3}
.balance-tx.minus .balance-amount{color:#ff8585}
.balance-tx.adjust .balance-amount{color:#ffd66e}
.balance-amount{white-space:nowrap}
.balance-tx .small{margin-top:3px;overflow-wrap:anywhere}
@media(max-width:650px){.balance-tx{align-items:flex-start;flex-direction:column}.balance-amount{font-size:16px}}
</style>
'''
...
def inject_balance_history(response):
    if response.content_type and 'text/html' in response.content_type and response.status_code == 200:
        match = re.match(r'^/admin/user/(\d+)$', request.path)
        if match:
...
(function(){{
  ...
  setInterval(refreshBalanceHistory,5000);
                ...
                response.set_data(html.replace('</body>', card + '</body>', 1))
    return response
