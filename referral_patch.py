...
def capture_referral():
    ...
    if ref and not current_user():
        try:
            uid = int(ref)
            if User.query.get(uid): session['referrer_id'] = uid
        except (ValueError, TypeError):
            pass

@app.before_request
def attach_referral_after_login():
    uid = session.get('user_id'); refid = session.get('referrer_id')
    if not uid or not refid or uid == refid: return
    if Referral.query.filter_by(referred_user_id=uid).first():
        session.pop('referrer_id', None); return
    inviter = db.session.get(User, refid); referred = db.session.get(User, uid)
    if inviter and referred:
        try:
            db.session.add(Referral(inviter_id=inviter.id, referred_user_id=referred.id)); db.session.commit()
...
def sync_referral_rewards(user_id):
    ...
    if not referral: return 0
    accepted_count = EmailSubmission.query.join(Batch).filter(Batch.user_id == user_id, EmailSubmission.status == 'accepted').count()
    if accepted_count <= referral.rewarded_emails: return 0
    ...
    db.session.add(Transaction(user_id=referral.inviter.id, amount=bonus, kind='referral_bonus', note=f'Bonus referral {delta} email dari {referral.referred_user.username}'))
    db.session.commit()
    return delta
...
def auto_sync_referral():
    ...
    if u and request.endpoint != 'static':
        try: sync_referral_rewards(u.id)
...
def sync_after_admin_review():
    if current_user() and current_user().username == __import__('os').getenv('ADMIN_USERNAME', 'admin') and request.endpoint in ('accept', 'reject', 'bulk_review', 'batch_bulk'):
        try:
            for r in Referral.query.all(): sync_referral_rewards(r.referred_user_id)
        except Exception: db.session.rollback()
...
@login_required
def referral_page():
    u = current_user(); link = request.url_root.rstrip('/') + f'/register?ref={u.id}'
    ...
    rows = ''.join(f'<tr><td>{r.referred_user.username}</td><td>{r.referred_user.id}</td><td>{r.rewarded_emails}</td><td>Rp {Decimal(r.total_reward):,.0f}</td></tr>' for r in referrals)
...
def patched_withdraw():
    ...
    if request.method == 'POST':
        ...
        if amount < MIN_WITHDRAW:
            ...
            flash('Maksimal withdraw adalah Rp 2.000.000.')
            ...
            db.session.commit(); flash(f'Withdraw #{w.id} berhasil diajukan ke admin.')
