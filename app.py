...
*{box-sizing:border-box}body{margin:0;font-family:Inter,system-ui,-apple-system,sans-serif;background:#f5f7fb;color:#172033}a{text-decoration:none;color:inherit;cursor:pointer}.nav{min-height:64px;background:#fff;border-bottom:1px solid #e8ebf1;display:flex;align-items:center;justify-content:space-between;padding:0 6%;position:sticky;top:0;z-index:50}.brand{font-weight:800;letter-spacing:-.4px}.navlinks{display:flex;gap:8px;align-items:center;font-size:14px;color:#5c6678;flex-wrap:wrap}.navlinks a{padding:9px 10px}.wrap{max-width:1120px;margin:32px auto;padding:0 20px}.hero{background:#fff;border:1px solid #e7eaf0;border-radius:22px;padding:30px;box-shadow:0 8px 30px #17203308}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px}.card{background:#fff;border:1px solid #e7eaf0;border-radius:18px;padding:20px;box-shadow:0 6px 24px #17203308}.muted{color:#697386}.big{font-size:30px;font-weight:800}.btn{border:0;border-radius:11px;padding:11px 16px;background:#172033;color:#fff;font-weight:700;cursor:pointer;display:inline-block}.btn.secondary{background:#eef1f6;color:#172033}.btn.danger{background:#a61b1b;color:#fff}.btn.success{background:#176b3a;color:#fff}.input,textarea{width:100%;padding:12px 13px;border:1px solid #dfe3ea;border-radius:11px;margin:7px 0 14px;background:#fff;font:inherit}textarea{min-height:130px;resize:vertical}.table{width:100%;border-collapse:collapse}.table th,.table td{padding:12px 8px;border-bottom:1px solid #edf0f4;text-align:left;font-size:14px;vertical-align:middle}.pill{padding:5px 9px;border-radius:999px;font-size:12px;font-weight:700;background:#eef1f6}.pending{background:#fff3cd}.accepted{background:#dff6e8;color:#176b3a}.rejected{background:#fde3e3;color:#9a2020}.banned{background:#fde3e3;color:#9a2020}.approved{background:#dff6e8;color:#176b3a}.flash{padding:12px 14px;border-radius:12px;background:#fff3cd;margin-bottom:14px}.top{display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap}.actions{display:flex;gap:8px;flex-wrap:wrap}.auth{max-width:440px;margin:70px auto}.small{font-size:13px}.rule{white-space:pre-wrap;line-height:1.7}.email-list{min-height:180px}.admin{background:#101827;color:#fff}.admin .nav{background:#101827;color:#fff;border-color:#263247}.admin .navlinks{color:#c8d0dd}.admin .navlinks a{color:#c8d0dd}.admin .card,.admin .hero{background:#172236;border-color:#293750}.admin .muted{color:#aab5c7}.admin .input,.admin textarea{background:#111a2a;color:#fff;border-color:#34445f}.admin .table th,.admin .table td{border-color:#293750}.check{width:18px;height:18px}.danger-box{border:1px solid #7f2424;background:#32171b;border-radius:14px;padding:14px}.statlink{display:block}.statlink:hover{opacity:.85}.status-box{border-radius:12px;padding:12px;background:#f7f8fb;margin-top:10px}.admin .status-box{background:#111a2a}@media(max-width:650px){.nav{padding:10px 12px;align-items:flex-start}.navlinks{gap:2px}.navlinks a{padding:8px 5px}.wrap{margin:20px auto}.hero{padding:22px}.table{display:block;overflow:auto;white-space:nowrap}}
</style></head><body class="{{'admin' if admin else ''}}"><nav class="nav"><a class="brand" href="{{url_for('dashboard' if current_user else 'home')}}">EMAIL MARKET</a><div class="navlinks">{% if current_user %}<a href="{{url_for('dashboard')}}">Dashboard</a><a href="{{url_for('submit')}}">Ajukan</a><a href="{{url_for('withdraw')}}">Withdraw</a><a href="{{url_for('rules')}}">Aturan</a>{% if current_user.username==admin_user %}<a href="{{url_for('admin_dashboard')}}">Admin</a>{% endif %}<a href="{{url_for('logout')}}">Keluar</a>{% else %}<a href="{{url_for('login')}}">Masuk</a><a class="btn" href="{{url_for('register')}}">Daftar</a>{% endif %}</div></nav><main class="wrap">{% with messages=get_flashed_messages() %}{% for m in messages %}<div class="flash">{{m}}</div>{% endfor %}{% endwith %}{{body|safe}}</main></body></html>'''
...
def migrate_schema():
    if db_url.startswith('sqlite'):
        try:db.session.execute(text('ALTER TABLE user ADD COLUMN is_banned BOOLEAN NOT NULL DEFAULT 0'));db.session.commit()
        except Exception:db.session.rollback()
    else:
        try:
            db.session.execute(text('ALTER TABLE "user" ADD COLUMN IF NOT EXISTS is_banned BOOLEAN NOT NULL DEFAULT FALSE'));db.session.commit()
        except Exception:db.session.rollback()
        try:
            db.session.execute(text('CREATE UNIQUE INDEX IF NOT EXISTS uq_email_submission_email ON email_submission (email)'));db.session.commit()
        except Exception:db.session.rollback()
...
@login_required
def dashboard():
    u=current_user();batches=Batch.query.filter_by(user_id=u.id).order_by(Batch.id.desc()).limit(20).all();tx=Transaction.query.filter_by(user_id=u.id).order_by(Transaction.id.desc()).limit(10).all();emails=EmailSubmission.query.join(Batch).filter(Batch.user_id==u.id).order_by(EmailSubmission.id.desc()).limit(100).all();withdrawals=Withdrawal.query.filter_by(user_id=u.id).order_by(Withdrawal.id.desc()).limit(10).all()
    rows=''.join(f'<tr><td>#{b.id}</td><td>{b.created_at}</td><td>{len(b.emails)}</td><td>{sum(e.status=="accepted" for e in b.emails)}</td><td>{sum(e.status=="rejected" for e in b.emails)}</td><td>Rp {sum(Decimal(e.payout or 0) for e in b.emails):,.0f}</td></tr>' for b in batches)
    email_rows=''.join(f'<tr><td>{e.email}</td><td>#{e.batch_id}</td><td><span class="pill {e.status}">{e.status}</span></td><td>Rp {Decimal(e.payout or 0):,.0f}</td><td>{e.rejection_reason or "-"}</td><td>{e.reviewed_at or "-"}</td></tr>' for e in emails)
    wd_rows=''.join(f'<tr><td>#{w.id}</td><td>{w.created_at}</td><td>Rp {Decimal(w.amount):,.0f}</td><td>{w.destination}</td><td><span class="pill {w.status}">{w.status}</span></td><td>{w.admin_note or "-"}</td></tr>' for w in withdrawals)
    body=f'''<div class="top"><div><div class="muted small">Halo, {u.username}</div><h1>Dashboard</h1></div><div class="actions"><a class="btn" href="/submit">+ Ajukan Email</a><a class="btn secondary" href="/withdraw">Withdraw</a></div></div><div class="grid"><div class="card"><div class="muted">Saldo tersedia</div><div class="big">Rp {Decimal(u.balance):,.0f}</div></div><div class="card"><div class="muted">Harga aktif / email</div><div class="big">Rp {price():,.0f}</div></div><div class="card"><div class="muted">Total batch</div><div class="big">{len(u.batches)}</div></div></div><div class="card" style="margin-top:16px"><h3>Riwayat Pengajuan</h3><table class="table"><tr><th>Batch</th><th>Tanggal</th><th>Email</th><th>Diterima</th><th>Ditolak</th><th>Payout</th></tr>{rows or '<tr><td colspan="6" class="muted">Belum ada pengajuan.</td></tr>'}</table></div><div class="card" style="margin-top:16px"><h3>Detail Semua Email</h3><p class="muted small">Email yang ditolak tetap tercatat di sini beserta alasan penolakannya.</p><table class="table"><tr><th>Email</th><th>Batch</th><th>Status</th><th>Payout</th><th>Alasan</th><th>Review</th></tr>{email_rows or '<tr><td colspan="6" class="muted">Belum ada email.</td></tr>'}</table></div><div class="card" style="margin-top:16px"><div class="top"><h3>Riwayat Withdraw</h3><a class="btn secondary" href="/withdraw">Ajukan Withdraw</a></div><table class="table"><tr><th>ID</th><th>Tanggal</th><th>Nominal</th><th>Tujuan</th><th>Status</th><th>Catatan Admin</th></tr>{wd_rows or '<tr><td colspan="6" class="muted">Belum ada pengajuan withdraw.</td></tr>'}</table></div><div class="card" style="margin-top:16px"><h3>Transaksi Terbaru</h3><table class="table"><tr><th>Tanggal</th><th>Jenis</th><th>Nominal</th><th>Catatan</th></tr>{''.join(f'<tr><td>{t.created_at}</td><td>{t.kind}</td><td>Rp {Decimal(t.amount):,.0f}</td><td>{t.note}</td></tr>' for t in tx) or '<tr><td colspan="4" class="muted">Belum ada transaksi.</td></tr>'}</table></div>'''
...
def submit():
    if request.method=='POST':
        ...
        if existing:
            sample=', '.join(sorted(existing)[:5]);more=' dan lainnya' if len(existing)>5 else '';flash(f'Pengajuan ditolak: email sudah pernah diajukan/dijual: {sample}{more}.');return redirect(url_for('submit'))
...
def withdraw():
    u=current_user()
    if request.method=='POST':
        try:amount=Decimal(request.form.get('amount','0')).quantize(Decimal('0.01'))
        except InvalidOperation:amount=Decimal('0')
        destination=request.form.get('destination','').strip()[:255]
        note=request.form.get('note','').strip()[:500]
        if amount<=0:flash('Nominal withdraw harus lebih dari 0.')
        elif not destination:flash('Masukkan tujuan withdraw, misalnya nomor rekening/e-wallet.')
        elif amount>Decimal(u.balance):flash('Saldo tidak mencukupi untuk withdraw tersebut.')
        else:
            ...
            db.session.add(w);db.session.flush();db.session.add(Transaction(user_id=u.id,amount=-amount,kind='withdrawal_pending',note=f'Withdraw #{w.id} diajukan'))
            db.session.commit();flash(f'Pengajuan withdraw #{w.id} berhasil dikirim ke admin. Saldo dicadangkan sampai admin memprosesnya.');return redirect(url_for('withdraw'))
    items=Withdrawal.query.filter_by(user_id=u.id).order_by(Withdrawal.id.desc()).limit(50).all()
    rows=''.join(f'<tr><td>#{w.id}</td><td>{w.created_at}</td><td>Rp {Decimal(w.amount):,.0f}</td><td>{w.destination}</td><td><span class="pill {w.status}">{w.status}</span></td><td>{w.note or "-"}</td><td>{w.admin_note or "-"}</td></tr>' for w in items)
    body=f'''<div class="top"><div><div class="muted">SALDO · WITHDRAW</div><h1>Ajukan Withdraw</h1><p class="muted">Saldo saat ini: <b>Rp {Decimal(u.balance):,.0f}</b>. Pengajuan akan diperiksa dan diproses manual oleh admin.</p></div><a class="btn secondary" href="/dashboard">← Kembali</a></div><div class="card"><form method="post"><label>Nominal Withdraw (Rp)</label><input class="input" name="amount" type="number" min="1" step="0.01" max="{Decimal(u.balance):.2f}" required placeholder="50000"><label>Tujuan Withdraw</label><input class="input" name="destination" required placeholder="DANA / GoPay / rekening / tujuan pembayaran"><label>Catatan (opsional)</label><textarea class="input" name="note" placeholder="Contoh: DANA 08xxxxxxxxxx"></textarea><button class="btn">Ajukan ke Admin</button></form></div><div class="card" style="margin-top:16px"><h3>Riwayat Withdraw</h3><table class="table"><tr><th>ID</th><th>Tanggal</th><th>Nominal</th><th>Tujuan</th><th>Status</th><th>Catatan User</th><th>Catatan Admin</th></tr>{rows or '<tr><td colspan="7" class="muted">Belum ada pengajuan withdraw.</td></tr>'}</table></div>'''
    return page(body,'Withdraw')

@app.route('/admin')
@admin_required
def admin_dashboard():
    users=User.query.order_by(User.id.desc()).all();batches=Batch.query.order_by(Batch.id.desc()).limit(50).all();pending=EmailSubmission.query.filter_by(status='pending').count();pending_w=Withdrawal.query.filter_by(status='pending').count()
    body=f'''<div class="top"><div><div class="muted">ADMIN PANEL</div><h1>Overview</h1></div><div class="actions"><a class="btn secondary" href="/admin/pending">Review Email</a><a class="btn secondary" href="/admin/withdrawals">Withdraw {pending_w and f"({pending_w})" or ""}</a><a class="btn secondary" href="/admin/settings">Pengaturan</a><a class="btn secondary" href="/admin/logs">Log</a></div></div><div class="grid"><div class="card"><div class="muted">Pengguna</div><div class="big">{len(users)}</div></div><div class="card"><div class="muted">Batch</div><div class="big">{Batch.query.count()}</div></div><div class="card"><div class="muted">Email menunggu review</div><div class="big">{pending}</div></div><div class="card"><div class="muted">Withdraw pending</div><div class="big">{pending_w}</div></div></div><div class="card" style="margin-top:16px"><div class="top"><h3>Semua Pengajuan</h3><a class="btn secondary" href="/admin/pending">Lihat Pending</a></div><table class="table"><tr><th>Batch</th><th>User</th><th>ID User</th><th>Email</th><th>Diterima</th><th>Ditolak</th><th>Aksi</th></tr>{''.join(f'<tr><td>#{b.id}</td><td><a href="/admin/user/{b.user.id}"><u>{b.user.username}</u></a></td><td>{b.user.id}</td><td>{len(b.emails)}</td><td>{sum(e.status=="accepted" for e in b.emails)}</td><td>{sum(e.status=="rejected" for e in b.emails)}</td><td><a class="btn secondary" href="/admin/batch/{b.id}">Review</a></td></tr>' for b in batches) or '<tr><td colspan="7" class="muted">Belum ada pengajuan.</td></tr>'}</table></div><div class="card" style="margin-top:16px"><div class="top"><h3>Pengguna</h3><a class="btn secondary" href="/admin/users">Kelola Semua User</a></div><table class="table"><tr><th>Username</th><th>ID</th><th>Status</th><th>Saldo</th><th>Batch</th><th>Aksi</th></tr>{''.join(f'<tr><td><a href="/admin/user/{u.id}"><u>{u.username}</u></a></td><td>{u.id}</td><td><span class="pill {"banned" if u.is_banned else "accepted"}">{"BANNED" if u.is_banned else "AKTIF"}</span></td><td>Rp {Decimal(u.balance):,.0f}</td><td>{len(u.batches)}</td><td><a class="btn secondary" href="/admin/user/{u.id}">Kelola</a></td></tr>' for u in users)}</table></div>'''
...
@admin_required
def bulk_review():
    action=request.form.get('action');ids=request.form.getlist('ids');q=EmailSubmission.query.filter_by(status='pending')
    ...
    if action=='accept':
        count=0
        for e in items:e.status='accepted';e.payout=e.batch.price_snapshot;e.reviewed_at=db.func.now();e.batch.user.balance=Decimal(e.batch.user.balance)+Decimal(e.payout);db.session.add(Transaction(user_id=e.batch.user.id,amount=e.payout,kind='payout',note=f'Email diterima · Batch #{e.batch_id}'));count+=1
        audit('bulk_accept',f'{count} email accepted');db.session.commit();flash(f'{count} email berhasil diterima dan saldo ditambahkan.')
    elif action=='reject':
...
@admin_required
def admin_batch(bid):
    b=db.get_or_404(Batch,bid);actions=''
    if any(e.status=='pending' for e in b.emails):actions=f'''<form method="post" action="/admin/batch/{b.id}/bulk"><div class="actions"><button class="btn success" name="action" value="accept">Terima Semua Pending</button><button class="btn danger" name="action" value="reject">Tolak Semua Pending</button></div></form>'''
...
def batch_bulk(bid):
    ...
    if action=='accept':
        for e in items:e.status='accepted';e.payout=b.price_snapshot;e.reviewed_at=db.func.now();b.user.balance=Decimal(b.user.balance)+Decimal(e.payout);db.session.add(Transaction(user_id=b.user.id,amount=e.payout,kind='payout',note=f'Email diterima · Batch #{b.id}'))
...
def admin_user(uid):
    ...
    if request.method=='POST':
        ...
        if action=='balance':
            ...
            else:u.balance=Decimal(u.balance)+amount;db.session.add(Transaction(user_id=u.id,amount=amount,kind='manual_adjustment',note=note or 'Penyesuaian saldo manual'));audit('balance_adjustment',f'User {u.username} ({u.id}) amount {amount}');db.session.commit();flash('Saldo berhasil diperbarui.')
        elif action=='ban':
    ...
    tx=Transaction.query.filter_by(user_id=u.id).order_by(Transaction.id.desc()).limit(50).all();batches=Batch.query.filter_by(user_id=u.id).order_by(Batch.id.desc()).limit(50).all();withdrawals=Withdrawal.query.filter_by(user_id=u.id).order_by(Withdrawal.id.desc()).limit(50).all();emails=EmailSubmission.query.join(Batch).filter(Batch.user_id==u.id).order_by(EmailSubmission.id.desc()).limit(100).all()
    body=f'''<div class="top"><div><div class="muted">USER #{u.id}</div><h1>{u.username}</h1><p>Status: <span class="pill {"banned" if u.is_banned else "accepted"}">{"BANNED" if u.is_banned else "AKTIF"}</span> · Saldo: <b>Rp {Decimal(u.balance):,.0f}</b></p></div><a class="btn secondary" href="/admin/users">← Kembali ke Pengguna</a></div><div class="grid"><div class="card"><h3>Tambah / Kurangi Saldo</h3><form method="post"><input type="hidden" name="action" value="balance"><label>Nominal (gunakan minus untuk mengurangi)</label><input class="input" name="amount" type="number" step="0.01" required placeholder="5000"><label>Alasan</label><input class="input" name="note" required placeholder="Koreksi data"><button class="btn">Simpan Saldo</button></form></div><div class="card"><h3>Status Akun</h3><p class="muted">User ID: <b>{u.id}</b><br>Username: <b>{u.username}</b><br>Terdaftar: {u.created_at}</p>{('<div class="danger-box"><b>User sedang diblokir.</b><form method="post" style="margin-top:10px"><input type="hidden" name="action" value="unban"><button class="btn success">Buka Blokir</button></form></div>' if u.is_banned else ('<div class="danger-box"><b>Blokir user</b><p class="small">User tidak bisa login atau mengajukan email saat diblokir.</p><form method="post"><input type="hidden" name="action" value="ban"><button class="btn danger">Ban User</button></form></div>' if u.username!=os.getenv('ADMIN_USERNAME','admin') else '<p class="muted">Akun admin tidak dapat diblokir.</p>'))}</div></div><div class="card" style="margin-top:16px"><div class="top"><h3>Riwayat Pengajuan User</h3><a class="btn secondary" href="/admin">← Kembali</a></div><table class="table"><tr><th>Batch</th><th>Tanggal</th><th>Email</th><th>Diterima</th><th>Ditolak</th><th>Pending</th><th>Aksi</th></tr>{''.join(f'<tr><td>#{b.id}</td><td>{b.created_at}</td><td>{len(b.emails)}</td><td>{sum(e.status=="accepted" for e in b.emails)}</td><td>{sum(e.status=="rejected" for e in b.emails)}</td><td>{sum(e.status=="pending" for e in b.emails)}</td><td><a class="btn secondary" href="/admin/batch/{b.id}">Lihat</a></td></tr>' for b in batches) or '<tr><td colspan="7" class="muted">Belum ada pengajuan.</td></tr>'}</table></div><div class="card" style="margin-top:16px"><h3>Detail Email User</h3><table class="table"><tr><th>Email</th><th>Batch</th><th>Status</th><th>Payout</th><th>Alasan</th><th>Review</th></tr>{''.join(f'<tr><td>{e.email}</td><td>#{e.batch_id}</td><td><span class="pill {e.status}">{e.status}</span></td><td>Rp {Decimal(e.payout or 0):,.0f}</td><td>{e.rejection_reason or "-"}</td><td>{e.reviewed_at or "-"}</td></tr>' for e in emails) or '<tr><td colspan="6" class="muted">Belum ada email.</td></tr>'}</table></div><div class="card" style="margin-top:16px"><h3>Riwayat Withdraw User</h3><table class="table"><tr><th>ID</th><th>Tanggal</th><th>Nominal</th><th>Tujuan</th><th>Status</th><th>Catatan</th><th>Aksi</th></tr>{''.join(f'<tr><td>#{w.id}</td><td>{w.created_at}</td><td>Rp {Decimal(w.amount):,.0f}</td><td>{w.destination}</td><td><span class="pill {w.status}">{w.status}</span></td><td>{w.note or "-"}<br>{w.admin_note or ""}</td><td><a class="btn secondary" href="/admin/withdrawals">Kelola</a></td></tr>' for w in withdrawals) or '<tr><td colspan="7" class="muted">Belum ada withdraw.</td></tr>'}</table></div><div class="card" style="margin-top:16px"><h3>Riwayat Transaksi</h3><table class="table"><tr><th>Tanggal</th><th>Nominal</th><th>Jenis</th><th>Catatan</th></tr>{''.join(f'<tr><td>{t.created_at}</td><td>Rp {Decimal(t.amount):,.0f}</td><td>{t.kind}</td><td>{t.note}</td></tr>' for t in tx) or '<tr><td colspan="4" class="muted">Belum ada transaksi.</td></tr>'}</table></div>'''
    return page(body,'Kelola User',True)

@app.route('/admin/withdrawals')
@admin_required
def admin_withdrawals():
    items=Withdrawal.query.order_by(Withdrawal.id.desc()).limit(200).all()
    rows=''.join(f'''<tr><td>#{w.id}</td><td><a href="/admin/user/{w.user.id}"><u>{w.user.username}</u></a></td><td>{w.user.id}</td><td>{w.created_at}</td><td>Rp {Decimal(w.amount):,.0f}</td><td>{w.destination}</td><td><span class="pill {w.status}">{w.status}</span></td><td>{w.note or '-'}</td><td>{w.admin_note or '-'}</td><td>{("<form method='post' action='/admin/withdrawal/%d/approve' style='display:inline'><button class='btn success'>Terima</button></form> <form method='post' action='/admin/withdrawal/%d/reject' style='display:inline'><input class='input' style='width:150px;margin:0' name='reason' placeholder='Alasan'><button class='btn danger'>Tolak</button></form>"%(w.id,w.id) if w.status=='pending' else '-')}</td></tr>''' for w in items)
    body=f'''<div class="top"><div><div class="muted">ADMIN · WITHDRAW</div><h1>Pengajuan Withdraw</h1><p class="muted">Saldo user sudah dicadangkan saat pengajuan. Jika ditolak, saldo otomatis dikembalikan.</p></div><a class="btn secondary" href="/admin">← Kembali</a></div><div class="card"><table class="table"><tr><th>ID</th><th>User</th><th>User ID</th><th>Tanggal</th><th>Nominal</th><th>Tujuan</th><th>Status</th><th>Catatan User</th><th>Catatan Admin</th><th>Aksi</th></tr>{rows or '<tr><td colspan="10" class="muted">Belum ada pengajuan withdraw.</td></tr>'}</table></div>'''
    return page(body,'Withdraw',True)

@app.post('/admin/withdrawal/<int:wid>/approve')
@admin_required
def approve_withdrawal(wid):
    w=db.get_or_404(Withdrawal,wid)
    if w.status!='pending':flash('Pengajuan withdraw sudah diproses.')
    else:
        w.status='approved';w.admin_note='Withdraw disetujui dan dapat dibayarkan secara manual oleh admin.';w.reviewed_at=db.func.now();audit('approve_withdrawal',f'Withdraw #{w.id} user {w.user.username} ({w.user.id}) amount {w.amount}');db.session.commit();flash(f'Withdraw #{w.id} disetujui.')
    return redirect(url_for('admin_withdrawals'))

@app.post('/admin/withdrawal/<int:wid>/reject')
@admin_required
def reject_withdrawal(wid):
    w=db.get_or_404(Withdrawal,wid)
    if w.status!='pending':flash('Pengajuan withdraw sudah diproses.')
    else:
        w.status='rejected';w.admin_note=request.form.get('reason','Withdraw ditolak oleh admin.')[:500];w.reviewed_at=db.func.now();w.user.balance=Decimal(w.user.balance)+Decimal(w.amount);db.session.add(Transaction(user_id=w.user.id,amount=w.amount,kind='withdrawal_refund',note=f'Pengembalian dana withdraw #{w.id}'));audit('reject_withdrawal',f'Withdraw #{w.id} user {w.user.username} ({w.user.id}) refunded {w.amount}');db.session.commit();flash(f'Withdraw #{w.id} ditolak dan saldo user dikembalikan.')
    return redirect(url_for('admin_withdrawals'))
...
with app.app_context():
    db.create_all()
    migrate_schema()
    if not db.session.get(Setting,'email_price'):db.session.add(Setting(key='email_price',value=os.getenv('DEFAULT_EMAIL_PRICE','500')))
    if not Rule.query.first():db.session.add(Rule(content=rules_text()))
    admin_username=os.getenv('ADMIN_USERNAME','admin')
    if not User.query.filter_by(username=admin_username).first():db.session.add(User(username=admin_username,password_hash=generate_password_hash(os.getenv('ADMIN_PASSWORD','change-me-now'))))
    db.session.commit()
